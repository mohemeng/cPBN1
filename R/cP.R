...
dcpois <- function(x, lambda){
  ...
  suppressMessages(require(cubature))
  bef <- function(y){
    g <- function(x){
...
rcpois <- function(n, lambda){
  ...
  for(i in bb){
   ...
   suppressWarnings(ans.roots[i] <- uniroot(func, lower =  0.1, upper = 50, extendInt = "yes")$root)
  }
  return(ans.roots)
}
...
qcpois <- function(p, lambda){
 ...
 suppressMessages(require(rootSolve))
  ...
  for(i in bb){
    func <- function(x){
      f <- pgamma(lambda, x, 1, lower.tail = FALSE) - p[i]
      return(f)
    }
    suppressWarnings(ans.roots[i] <- uniroot(func, lower =  0.1, upper = 50, extendInt = "yes")$root)
  }
...
cpoisMME <- function(data){
  ...
  return(integrate(func, lower = 0, upper = Inf )$value - mean(data))
  }

  suppressWarnings(val.lambda <- uniroot(parfunc, lower =  0.1, upper = 50, extendInt = "yes")$root)
  return(val.lambda)
}
